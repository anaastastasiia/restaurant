...
export const useCartStoreTest = create<CartState>((set, get) => ({
  ...
  addToCart: (item: Item) => {
    ...
    if (existingItem) {
    console.log("ISTNIEJE przed dodaniem: ", get().cartItems);
    set((state) => ({
      ...
      console.log("po dodaniu: ", get().cartItems);
    } else {
      console.log("NIE ISTN przed dodaniem: ", get().cartItems);
      set((state) => ({
        cartItems: [...state.cartItems, { ...item, count: 1, price: item.price }],
      }));
      console.log("po dodaniu: ", get().cartItems);
    }
  },
  ...
  updateItemCount: (itemId: string, newCount: number, newPrice: string) => {
    console.log("update: ", get().cartItems);
    set((state) => ({
